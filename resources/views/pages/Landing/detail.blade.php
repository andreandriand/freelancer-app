                    ...
                    <div x-data="{ tab: window.location.hash ? window.location.hash.substring(1) : 'description' }" id="tab_wrapper">
                        ...
                        <div x-show.transition.duration.500ms="tab === 'description'" class="leading-8 text-md">
                            ...
                            <ul class="mb-4 list-check">
                                @forelse ($advantage_service as $as)
                                    <li class="pl-10 my-2">{{ $as->advantage }}</li>
                                @empty
                                    {{-- empty --}}
                                @endforelse
                            </ul>
                        ...
                        <div x-show.transition.duration.500ms="tab === 'seller'" class="leading-8 text-md">
                            ...
                            <ul class="mb-8 list-check">
                                @forelse ($advantage_user as $au)
                                    <li class="pl-10 my-2">{{ $au->advantage }}</li>
                                @empty
                                    {{-- empty --}}
                                @endforelse
                            </ul>
                            ...
                            <div class="mb-8 skills">
                                @forelse ($tagline as $tag)
                                    <span
                                        class="inline-block px-3 py-1 mr-2 rounded bg-serv-services-bg">{{ $tag->tagline }}</span>
                                @empty
                                    {{-- empty --}}
                                @endforelse
                            </div>
                    ...
                    <div class="flex items-start px-4 pt-6">
                        @if ($service->user->detail_user->photo != null)
                            <img class="object-cover w-16 h-16 mr-4 rounded-full"
                                src="{{ url(Storage::url($service->user->detail_user->photo)) }}" alt="user photo"
                                loading="lazy" />
                        @else
                            <img class="object-cover w-16 h-16 mr-4 rounded-full" src="{{ asset('assets/images/pp.svg') }}"
                                alt="Profile Photo">
                        @endif
                        <div class="w-full">
